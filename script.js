...
if(walkGallery){
  ['photo_2026-08-28_06-35-48.jpg','photo_2026-08-28_06-35-53.jpg','photo_2026-08-28_06-36-18.jpg','photo_2026-08-28_06-36-22.jpg','photo_2026-08-28_06-36-24.jpg','photo_2026-08-28_06-38-07.jpg'].forEach(name=>{
    ...
    img.src='assets/'+name;img.alt='Pet House guest enjoying a walk';img.loading='lazy';
...
if(contactSection&&!document.querySelector('#booking-form')){
  ...
  const buildRequest=()=>{const data=new FormData(form);return 'Pet House booking request\n\nPet name: '+(data.get('Pet name')||'')+'\nPet type: '+(data.get('Pet type')||'')+'\nAge: '+(data.get('Age')||'')+'\nBreed/type: '+(data.get('Breed / type')||'Not specified')+'\nCheck-in: '+(data.get('Check-in')||'')+'\nCheck-out: '+(data.get('Check-out')||'')+'\nSpecial care: '+(data.get('Special care')||'None specified')+'\n\nOwner name: '+(data.get('Owner name')||'')+'\nBest contact: '+(data.get('Best contact')||'')+'\nVaccinations/parasite treatment confirmed: '+(data.get('Vaccinations confirmed')?'Yes':'Not confirmed');};

  form.addEventListener('submit',async e=>{
    e.preventDefault();
    if(!form.reportValidity())return;
    submitButton.disabled=true;submitButton.textContent='Sending…';status.className='form-status';status.textContent='';
    ...
    try{
      ...
      status.innerHTML='<strong>Thank you! Your request has been sent.</strong><span>We’ll get back to you shortly.</span>';
      submitButton.textContent='Sent ✓';
      setTimeout(()=>{submitButton.disabled=false;submitButton.textContent='Send booking request';},3000);
    }catch(error){
      status.className='form-status error';
      status.innerHTML='<strong>We couldn’t send your request.</strong><span>Please try again or message us on Telegram @pethouse_pp.</span>';
      submitButton.disabled=false;submitButton.textContent='Send booking request';
    }
  });

  formSection.querySelector('.copy-request').addEventListener('click',async e=>{if(!form.reportValidity())return;try{await navigator.clipboard.writeText(buildRequest());e.currentTarget.textContent='Copied ✓';setTimeout(()=>e.currentTarget.textContent='Copy request',1800);}catch(err){alert('Please copy the form details manually.');}});
